...
/**
 * Main Activity for App B
 * Demonstrates two-way IPC communication with App A using AIDL
 */
class MainActivityB : ComponentActivity() {

    companion object {
        private const val TAG = "MainActivityAppB"
        private const val REMOTE_PACKAGE = "cam.manoash.twoipcapps.appa"
        private const val REMOTE_SERVICE_CLASS = "$REMOTE_PACKAGE.EventServiceA"
        private const val REMOTE_SERVICE_ACTION = "cam.manoash.twoipcapps.appa.ipc.EventService"
    }
    ...
    // Services
    private var remoteService: IEventServiceA? = null
    private var localService: IEventServiceB? = null

    // AIDL callback for receiving events
    private val callback = object : IEventCallbackB.Stub() {
    ...
    private val remoteConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            remoteService = IEventServiceA.Stub.asInterface(service)
    ...
    private val localConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            localService = IEventServiceB.Stub.asInterface(service)
    ...
    private fun bindToLocalService() {
        val intent = Intent(this, EventServiceB::class.java)
